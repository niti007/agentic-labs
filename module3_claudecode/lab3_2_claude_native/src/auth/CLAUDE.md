...
Path-specific rule file for auth code. Claude Code loads it **only** when working on files
in `src/auth/`, so the strict rules stay relevant and don't pollute unrelated work.
